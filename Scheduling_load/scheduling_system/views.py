...
def get_instructor_schedule(request):
    ...
    try:
        # Query ProgramSchedule by instructor_name
        ...
        schedule_data = Schedule.objects.filter(program_schedule__in=program_schedules)

        # Format the schedule data
        courses = []  # Change to a flat list
        for entry in schedule_data:
            courses.append({
                'course_name': entry.program_schedule.course_name,
                'day': entry.day.lower(),  # Keep day as lowercase for consistency
